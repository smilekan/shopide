    ...
    public void whenOrdered_then_CREATE_ (@Payload Ordered ordered) {
        try {
            if (ordered.isMe()) {
                ...
                mypage.setProdId(ordered.getProductId());
    ...
    public void whenShipped_then_UPDATE_(@Payload Shipped shipped) {
        try {
            if (shipped.isMe()) {
                ...
                List<Mypage> mypageList = mypageRepository.findBy(shipped.get());
                for(Mypage mypage : mypageList){
                    // view 객체에 이벤트의 eventDirectValue 를 set 함
                    mypage.setDeliveryId(shipped.getId());
                    mypage.setStatus(shipped.getStatus());
                    mypage.set(shipped.get());
                    // view 레파지 토리에 save
                    mypageRepository.save(mypage);
                }
            }
        ...
        try {
            if (deliveryCanceled.isMe()) {
                ...
                for(Mypage mypage : mypageList){
                    ...
                    mypage.setOrderId(deliveryCanceled.getOrderId());
                    // view 레파지 토리에 save
                    mypageRepository.save(mypage);
                }
                List<Mypage> mypageList = mypageRepository.findBy(deliveryCanceled.get());
                for(Mypage mypage : mypageList){
                    // view 객체에 이벤트의 eventDirectValue 를 set 함
                    mypage.setStatus(deliveryCanceled.getStatus());
                    mypage.set(deliveryCanceled.get());
                    // view 레파지 토리에 save
                    mypageRepository.save(mypage);
                }
            }
